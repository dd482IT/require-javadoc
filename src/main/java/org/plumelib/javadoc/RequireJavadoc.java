...
public class RequireJavadoc {
    ...
    public static void main(String[] args) {
        ...
        Options options = new Options("java org.plumelib.javadoc.RequireJavadoc [options] [directory-or-file ...]", rj);
     ...
     */
    private void setJavaFiles( RequireJavadoc this,  String  [] args) {
        if (args.length == 0) {
    ...
    private class JavaFilesVisitor extends SimpleFileVisitor<Path> {
        ...
        public  FileVisitResult preVisitDirectory( JavaFilesVisitor this,  Path dir,  BasicFileAttributes attr) {
     ...
     */
    private   boolean shouldNotRequire( RequireJavadoc this,  String name) {
     ...
     */
    private   boolean shouldExclude( RequireJavadoc this,  String fileName) {
        if (exclude == null) {
            return false;
        }
     ...
     * @return true if the file or directory should be skipped
     */
    private   boolean shouldExclude( RequireJavadoc this,  Path path) {
    ...
    private enum PropertyKind {
         ...
         */
          boolean isGetter( PropertyKind this) {
            return this != SETTER;
        }
     ...
     */
    private   boolean isTrivialGetterOrSetter( RequireJavadoc this,  MethodDeclaration md) {
     ...
     * @return true if this method is a trivial getter or setter
     */
    private   boolean isTrivialGetterOrSetter( RequireJavadoc this,  MethodDeclaration md,  PropertyKind propertyKind) {
        String propertyName = propertyName(md, propertyKind);
        return propertyName != null && hasCorrectSignature(md, propertyKind, propertyName) && hasCorrectBody(md, propertyKind, propertyName);
    }
     ...
     */
    private  String propertyName( RequireJavadoc this,  MethodDeclaration md,  PropertyKind propertyKind) {
        String methodName = md.getNameAsString();
     ...
     */
    private   boolean hasCorrectSignature( RequireJavadoc this,  MethodDeclaration md,  PropertyKind propertyKind,  String propertyName) {
     ...
     * @return true if the body of the given method is a property accessor
     */
    private   boolean hasCorrectBody( RequireJavadoc this,  MethodDeclaration md,  PropertyKind propertyKind,  String propertyName) {
        ...
        if (propertyKind.isGetter()) {
            ...
            if (!(value instanceof NameExpr && ((NameExpr) value).getNameAsString().equals(propertyName))) {
                return false;
            }
            return true;
        } else {
     ...
     */
    private  Statement getOnlyStatement( RequireJavadoc this,  MethodDeclaration md) {
    ...
    private class RequireJavadocVisitor extends VoidVisitorAdapter<Void> {
         ...
         */
        private  String errorString( RequireJavadocVisitor this,  Node node,  String simpleName) {
            Optional<Range> range = node.getRange();
            if (range.isPresent()) {
                Position begin = range.get().begin;
                Path path = (relative ? (filename.isAbsolute() ? workingDirAbsolute : workingDirRelative).relativize(filename) : filename);
        ...
        public void visit( RequireJavadocVisitor this,  CompilationUnit cu,  Void ignore) {
            ...
            if (opd.isPresent()) {
                ...
                if (oTypeName.isPresent() && oTypeName.get().equals("package-info") && !hasJavadocComment(opd.get()) && !hasJavadocComment(cu)) {
         ...
         * @return true if this method is annotated with {@code @Override}
         */
        private   boolean isOverride( RequireJavadocVisitor this,  MethodDeclaration md) {
     ...
     * @return true if this node has a Javadoc comment
     */
    private   boolean hasJavadocComment( RequireJavadoc this,  Node n) {
        ...
        if (oc.isPresent() && (oc.get().isJavadocComment() || oc.get().getContent().startsWith("/**"))) {
            return true;
        }
        return false;
    }
